...
# Azure DevOps Get PR Comments Script - Wrapper for Bun implementation
# This script calls the Bun-based implementation in agent-tools/

# Source ~/.vars if it exists (for environment variables)
if [ -f ~/.vars ]; then
    source ~/.vars
fi
...
exec bun run "$AGENT_TOOLS_DIR/src/ado-get-pr-comments.ts" "$@"